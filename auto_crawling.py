...
def git_push():
    """job_postings.json 파일의 변경사항을 Git에 커밋하고 원격 저장소로 푸시"""
    try:
        # 변경사항 스테이징
        subprocess.run(["git", "add", DATA_FILE], check=True)
        # 커밋 메시지 작성 (필요에 따라 메시지 수정)
        subprocess.run(["git", "commit", "-m", "Update job postings"], check=True)
        # 원격 저장소로 푸시 (브랜치 이름을 환경에 맞게 수정, 여기서는 master 사용)
